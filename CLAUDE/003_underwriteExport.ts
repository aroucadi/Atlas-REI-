...
// ---------------------------------------------------------------------------
// 4. Route handler sketch (apps/api)
// ---------------------------------------------------------------------------
//
// router.get('/underwrites/:id/export.xlsx', async (req, res) => {
//   const payload = await buildUnderwriteExportPayload(req.params.id); // your DB -> UnderwriteExportPayload mapper
...
//   res.setHeader(
//     'Content-Disposition',
//     `attachment; filename="${payload.meta.propertyName.replace(/\s+/g, '_')}_underwrite.xlsx"`
//   );
