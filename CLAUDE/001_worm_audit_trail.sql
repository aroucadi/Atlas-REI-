...
-- ---------------------------------------------------------------------------
-- PRISMA SCHEMA NOTES (add to schema.prisma, do not run as SQL):
--
-- model AuditLog {
...
-- }
--
-- App-layer: never call prisma.auditLog.update() or .delete() — they will
-- throw at the DB level regardless, but fail fast in a service-layer guard
-- too so the error is legible (a raw Postgres exception is ugly in API logs).
-- Example guard in your audit-log service:
--
--   update(...): never {
--     throw new Error('AuditLog is append-only. Use create() only.');
--   }
-- ---------------------------------------------------------------------------
